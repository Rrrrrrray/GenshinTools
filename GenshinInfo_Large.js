...
// icon-color: cyan; icon-glyph: street-view;
/**
...
function bg_img() {
  ...
  const path = file.bookmarkedPath(filename)
  const img = file.readImage(path)
  return img
}
...
async function createWidget() {
  ...
  let transfmIcon = await loadTransfmIcon()
  let widget = new ListWidget()
  log(args.widgetParameter)  
  if(args.widgetParameter == null) {
    bg_gradient(widget)
  } else {
    widget.backgroundImage = await shadowImage(bg_img())
  } 
  // 布局
  //第一栏
  let topHorizon = widget.addStack()
  topHorizon.layoutHorizontally()
  widget.addSpacer(isLarge ? 10 : 5)
  //第二栏
  ...
  topHorizon.addSpacer(isSmall ? 2 : 10)
  ...
  // 参量质变仪
  let transfmStack = widget.addStack()
  let TransfmIconElement = transfmStack.addImage(transfmIcon)
  TransfmIconElement.imageSize = new Size(ThemeConfig.iconSize, ThemeConfig.iconSize)
  TransfmIconElement.cornerRadius = ThemeConfig.iconRadius
  transfmStack.addSpacer(ThemeConfig.iconSpacer)

  let transfmText = ''
  if (!isSmall) {
    transfmText = transfmStack.addText("参量质变仪：")
    transfmText.textColor = Color.white()
    transfmText.textOpacity = 1
    transfmText.font = Font.mediumSystemFont(ThemeConfig.textSize)
  }
  ...
  TransfmTextElement.font = Font.mediumSystemFont(ThemeConfig.textSize)
  transfmStack.centerAlignContent()
  widget.addSpacer(isLarge ? 40 : 10)
...
async function loadTransfmIcon() {
